...
const truncateToCharacters = (text: string, charLimit: number): string => {
  ...
  return (
    ...
    >
      ...
      {tours.map((tour) => {
        const imageUrl = typeof tour.overviewImage === 'object' && tour.overviewImage !== null && 'url' in tour.overviewImage 
          ? tour.overviewImage.url 
          : `/api/media/file/${tour.slug}-overview.webp`;
        const normalizedImageUrl = formatImageUrl(imageUrl) || ''

        return (
          <div key={tour.id} className="col-span-4 lg:col-span-3 flex flex-col bg-card shadow-sm border border-border group transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 overflow-hidden">
            {/* Image Section */}
