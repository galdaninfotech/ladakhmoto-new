...
function CarouselNavigation({ className, classNameButton, alwaysShow }: CarouselNavigationProps) {
  ...
  return (
    <div className={cn("absolute bottom-20 left-0 right-0 z-10 flex justify-start gap-2 px-4 container mx-auto pointer-events-none", className)}>
      <button
        type="button"
        aria-label="Previous slide"
        className={cn(
          "pointer-events-auto h-fit w-fit bg-gray-500 backdrop-blur-sm rounded-full p-3 transition-all duration-300 hover:bg-accent shadow-lg opacity-100",
          classNameButton,
        )}
      ...
      <div className="flex space-x-3">
        {Array.from({ length: itemsCount }, (_, i) => (
            <button
              ...
              onClick={() => setIndex(i)}
              className={cn(
                "w-5 h-5 rounded-full flex items-center justify-center transition-all duration-300 group/bullet",
                index === i ? "border border-white/60 bg-transparent" : "border border-transparent bg-transparent cursor-pointer",
                classNameButton,
              )}
            >
              <span 
                className={cn(
                  ...
                  index === i 
                    ...
                    : "w-1.5 h-1.5 bg-white/60 group-hover/bullet:bg-white"
                )} 
              />
            </button>
          ))}
